...
echo Building lua...
cc -O3 -Wall -DLUA_USE_POSIX -DLUA_USE_DLOPEN -fpic -c *.c
cc -shared -fpic -o liblua.so lapi.o lcode.o ldebug.o ldo.o ldump.o lfunc.o lgc.o llex.o lmem.o lobject.o lopcodes.o lparser.o lstate.o lstring.o ltable.o ltm.o lundump.o lvm.o lzio.o lauxlib.o lbaselib.o ldblib.o liolib.o lmathlib.o loslib.o ltablib.o lstrlib.o loadlib.o linit.o
cp liblua.so ${INSTALL_DIR}/

cd ${ROOT}/adclib 

echo Building adclib...
c++ -O3 -Wall -fpic -c -I$LUA_DIR *.cpp
c++ -shared -fpic -o adclib.so *.o -L$LUA_DIR -llua
...
cc -O3 -Wall -fpic -c -I$LUA_DIR *.c
cc -shared -fpic -o unicode.so *.o -L$LUA_DIR -llua
...
cc -O3 -c -Wall -fpic -I$LUA_DIR *.c
cc -shared -fpic -o socket.so *.o -L$LUA_DIR -llua
cp socket.so $INSTALL_DIR/lib/luasocket/socket/socket.so
mv mime.c.not mime.c
cc -O3 -fpic -Wall -c -I$LUA_DIR *.c
cc -shared -fpic -o mime.so *.o -L$LUA_DIR -llua
...
mv wsocket.c wsocket.c.not
cc -O3 -c -Wall -fpic -DOPENSSL_NO_HEARTBEATS -I$LUA_DIR *.c
cc -shared -fpic -o ssl.so *.o -L$LUA_DIR -llua -lssl -lcrypto
...
echo Building hub...
cc -O3 -Wall -I$LUA_DIR -c *.c
cc -o luadch *.o -L$LUA_DIR -L/opt/lib, -llua -lm -Wl,-rpath .
