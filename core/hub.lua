...
_identify = {
    ...
    BINF = function( user, adccmd )
        ...
        if not reguser and _cfg_reg_only then
            ...
            out_error( "Nick: ", nick, " is not registered.")  --sends the failed nick to error.log
            return true
        ...
        if reguser then
            ...
            if ( ( profile.badpassword or 0 ) >= _cfg_max_bad_password ) and ( sec < _cfg_bad_pass_timeout ) then
                ...
                out_error( "Nick: ", nick, " max password tries exceeded. Timeout in sec: ", sec) --sends to error.log if password tries exceeded
                return true
            end
...
_verify = {
    ...
    HPAS = function( user, adccmd )
        ...
        if ( userhash ~= hubhash ) and ( userhash ~= hubhashold ) then
            ...
            out_error( "Nick: ", nick, " bad password.") --erro.logr with nickname in case of bad pass
        else
